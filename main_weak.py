...
class Response:
    def __init__(self, context, duration, id, island_type, grade, participant, movement, opposition, category):
...
def get_all_files():
    participants = []
    directory_in_str = r"D:\IslandsPython\weak_islands_csv"
    ...
    with open(file_path, "r", encoding="UTF-8") as f:
        ...
        for line in reader:
            ...
            if line[1] == 'stimulus' and line[-7] != '':
                index_duration = 10
                index_number = line.index('679df971131ae225e46b4282')
                # if line[index_number-5].startswith('2024-05-2'):
                #     index_duration = 11

                # quit()
                # print(line)
                if new_participant.check_response_byid(int(line[index_number+11])):
                    continue
                # print(line[index_number + 7], line[index_duration], line[index_number + 11], line[index_number + 8],
                #       line[index_number + 4], new_participant, line[index_number + 12], line[index_number + 13],
                #       line[index_number + 14], sep='|')


                new_response = Response(line[index_number + 7], line[index_duration], line[index_number + 11], line[index_number + 8],
                      line[index_number + 4], new_participant, line[index_number + 12], line[index_number + 13],
                      line[index_number + 14])
            ...
            if line[0].startswith('{"labjs'):
                ...
                # if line[index_number - 5].startswith('2024-05-2'):
                #     index_number -= 1
                # print(line[index_number-17], line[index_number-15], line[index_number-16], line[index_number-3])
...
def create_file_participants(participants):
    with open('all_participants_weak.txt', 'w', encoding='UTF-8') as f:
        for i in participants:
            print('\n' + '+' * 180 + '\n', file=f)
...
def print_responses(responses, f=0):
    responses_summ = 0
    with open('stats_weak.txt', 'a', encoding='UTF-8') as f:
